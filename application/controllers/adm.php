...
class Adm extends CI_Controller {
	...
	public function m_siswa() {
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_siswa SET nama = '".bersih($p,"nama")."', nis = '".bersih($p,"nis")."', kelas = '".bersih($p,"kelas")."'	WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_siswa VALUES (null, '".bersih($p,"nama")."', '".bersih($p,"nis")."', '".bersih($p,"kelas")."')");
			}
			...
			$det_user = $this->db->query("SELECT id, nis FROM m_siswa WHERE id = '$uri4'")->row();

			if (!empty($det_user)) {
				$q_cek_username = $this->db->query("SELECT id FROM m_admin WHERE username = '".$det_user->nis."' AND level = 'siswa'")->num_rows();

				if ($q_cek_username < 1) {

					$this->db->query("INSERT INTO m_admin VALUES (null, '".$det_user->nis."', md5('".$det_user->nis."'), 'siswa', '".$det_user->id."')");
					$ret_arr['status'] 	= "ok";
			...
			$this->db->query("UPDATE m_admin SET password = md5('".$det_user->nis."') WHERE level = 'siswa' AND kon_id = '".$det_user->id."'");
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[] = '<div class="btn-group">
                          <a href="#" onclick="return m_siswa_e('.$d['id'].');" class="btn bg-yellow waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">create</i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_siswa_h('.$d['id'].');" class="btn bg-red waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">delete</i> &nbsp;&nbsp;Hapus</a>
                         ';

                if ($d['ada'] == "0") {
                  $data_ok[4] .= '<a href="#" onclick="return m_siswa_u('.$d['id'].');" class="btn bg-blue waves-effect waves-light"><i class="material-icons" style="margin-right: 16px; color: #fff">person</i> &nbsp;&nbsp;Aktifkan User</a>';
                } else {
                  $data_ok[4] .= '<a href="#" onclick="return m_siswa_ur('.$d['id'].');" class="btn bg-orange waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">shuffle</i> &nbsp;&nbsp;Reset Password</a>';
                }
		...
		if ($uri3 == "det") {
	        ...
	        foreach ($q_datanya as $d) {
				...
				$data_ok[3] = '<div class="btn-group">
							<a href="#" onclick="return m_guru_e('.$d['id'].');" class="btn bg-green waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">create</i> &nbsp;&nbsp;Edit</a>
							<a href="#" onclick="return m_guru_h('.$d['id'].');" class="btn bg-red waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">delete</i> &nbsp;&nbsp;Hapus</a>
							<a href="#" onclick="return m_guru_matkul('.$d['id'].');" class="btn bg-teal waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">book</i> &nbsp;&nbsp;Mapel Diampu</a>';
                if ($d['ada'] == "0") {

				  $data_ok[3] .= '<a href="#" onclick="return m_guru_u('.$d['id'].');" class="btn bg-blue waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; margin-right: 16px; color: #fff">person</i> &nbsp;&nbsp;Aktifkan User</a>';
                } else {
				  $data_ok[3] .= '<a href="#" onclick="return m_guru_ur('.$d['id'].');" class="btn bg-orange waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">shuffle</i> &nbsp;&nbsp;Reset Password</a>';
                }
		...
		if ($uri3 == "det") {
	        ...
	        foreach ($q_datanya as $d) {
				...
				$data_ok[2] = '<div class="btn-group">   
				        	<a href="#" onclick="return m_mapel_e('.$d['id'].');" class="btn bg-yellow waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">create</i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_mapel_h('.$d['id'].');" class="btn bg-red waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">delete</i> &nbsp;&nbsp;Hapus</a>
                         ';
	...
	public function m_soal() {
		...
		if ($uri3 == "det") {
			...
			if (!empty($data)) {
				...
				$data_ok[2] = $d['nmmapel'];
				$data_ok[3] = $d['nmguru'];
				$data_ok[4] = "Jml dipakai : ".($total)."<br>Benar: ".$jml_salah.", Salah: ".$jml_salah."<br>Persentase benar : ".number_format($persen_benar)." %";
				$data_ok[5] = '<div class="btn-group">
				  <a href="'.base_url().'adm/m_soal/edit/'.$d['id'].'" class="btn bg-yellow waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">create</i> &nbsp;&nbsp;Edit</a>
				  <a onclick="hapus_soal()" class="btn bg-red waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">delete</i> &nbsp;&nbsp;Hapus</a>
				  <script type="text/javascript">
					function hapus_soal(){
					  swal ({
						  title: "Yakin Hapus Data Soal?",
						  text: "Jika anda menghapus soal, maka data tidak dapat dikembalikan",
						  icon: "warning",
						  dangerMode: true,
						  buttons: ["Cancel", "Hapus"],
					  }).then(okay => {
						  if (okay) {
							  window.location.href = "'.base_url().'adm/m_soal/hapus/'.$d['id'].'";
		...
		if ($uri3 == "det") {
			...
			exit();			
		        ...
		        foreach ($q_datanya as $d) {
		            ...
		            $data_ok = array();
		            $data_ok[0] = $no++;
		            $data_ok[1] = $d['nama_ujian']."<br>Token : <b>".$d['token']."</b> &nbsp;&nbsp; <a href='#' onclick='return refresh_token(".$d['id'].")' title='Perbarui Token'><i class='material-icons'>cached</i></a>";
		            $data_ok[2] = $d['mapel'];
		            ...
		            $data_ok[6] = '
		            	<div class="btn-group">
                          <a href="#" onclick="return m_ujian_e('.$d['id'].');" class="btn btn-info btn-xs"><i class="material-icons" style="margin-left: 0px; color: #fff">edit</i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_ujian_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="material-icons" style="margin-left: 0px; color: #fff">delete</i> &nbsp;&nbsp;Hapus</a>
                        </div>
	                         ';
	...
	public function h_ujian() {
		...
		if ($uri3 == "det") {
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[5] = '<a href="'.base_url().'adm/h_ujian/batalkan_ujian/'.$d['id'].'/'.$this->uri->segment(4).'" class="btn bg-red waves-effect waves-light" onclick="return confirm(\'Anda yakin...?\');"><i class="material-icons">cancel</i> Batalkan Ujian</a>';
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[6] = '<a href="'.base_url().'adm/h_ujian/det/'.$d['id'].'" class="btn bg-blue waves-effect waves-light"><i class="material-icons" style="margin-left: 0px; color: #fff">search</i> &nbsp;&nbsp;Lihat Hasil</a>
                         ';
	...
	public function hasil_ujian_cetak() {
		...
		$tym = date('d-m-Y');
		$namafile = 'Laporan Hasil Ujian CBT - '.$tym.'.pdf';
		...
		$this->dompdf->stream($namafile,array('Attachment'=>0));
	...
	public function rubah_password() {
		...
		if ($uri3 == "simpan") {
			...
			if ($cek_pass_lama->password != $p1_md5) {
				...
				$ret['status'] = "error";
				$ret['msg'] = "Password baru minisal terdiri dari 6 huruf..";
 			} else {
	...
	public function sudah_selesai_ujian() {
		...
		if (empty($q_nilai)) {
			...
			$a['p'] = "v_selesai_ujian";
            $a['selesai_tgl']=tjs($q_nilai->tgl_selesai, "l");
            $a['selesai_jam']="";
            $a['selesai_nilai']="$q_nilai->nilai";
		}
		$this->load->view('aaa', $a);
	}
	/* Login Logout */
