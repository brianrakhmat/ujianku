...
function tampil_media($file,$width="320px",$height="240px") {
	...
	if (!in_array($eks, $eks_video) && !in_array($eks, $eks_audio) && !in_array($eks, $eks_image)) {
		...
		if (in_array($eks, $eks_image)) {
			if (is_file("./".$file)) {
				$ret .= '<div class="gambar"><img class="img-responsive" src="'.base_url().$file.'" style="width: '.$width.'; height: '.$height.'; display: inline; float: left"></div>';
			} else {
				$ret .= '';
			}
		}
...
function gen_menu() {
	...
	if ($sess_level == "guru") {
	  $menu = array(
	            ...
	            array("icon"=>"book", "url"=>"m_soal", "text"=>"Data Soal"),
	            array("icon"=>"assignment", "url"=>"m_ujian", "text"=>"Ujian"),
	            array("icon"=>"book", "url"=>"h_ujian", "text"=>"Hasil Ujian"),
	          );
	}  else if ($sess_level == "siswa") {
		$menu = array(
				  array("icon"=>"home", "url"=>"", "text"=>"Beranda"),
				  array("icon"=>"assignment", "url"=>"ikuti_ujian", "text"=>"Ujian"),
				);
	} else if ($sess_level == "admin") {
	  $menu = array(
	            array("icon"=>"home", "url"=>"", "text"=>"Beranda"),
	            array("icon"=>"person", "url"=>"m_siswa", "text"=>"Data Siswa"),
	            array("icon"=>"person", "url"=>"m_guru", "text"=>"Data Guru"),
	  ...
	  $menu = array(
	            array("icon"=>"dashboard", "url"=>"", "text"=>"Beranda")
	          );
	  if ($url == "ikut_ujian") {
	    $menu = null;
	  }
